...
def main():
    ...
    check=1
    os.mkdir(os.path.join(os.getcwd(),'laptop_pics'))

    for page in page_no_list:


        url=f'https://www.flipkart.com/search?q=laptop&otracker=search&otracker1=search&marketplace=FLIPKART&as-show=on&as=off&as-pos=1&as-type=HISTORY&page={page}'

        print('page_number= ',check)
        check=check+1
        imagedown(url,'laptop_pics')
